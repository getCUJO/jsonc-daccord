                                  ...
                                  json_object *joutput_node)
{
    ...
    json_object *jpatprop_node =
        _jdac_output_create_and_append_node(joutput_node, "patternProperties");
    ...
    json_object_object_foreach(jpatprops, jprop_key, jprop_val)
    {
        json_object_object_foreach(jobj, jobj_key, jobj_val)
        {
            ...
            if (ret == JDAC_REGEX_COMPILE_FAILED) {
                ...
                json_object *jpat_item = _jdac_output_create_and_append_node_concatnames(
                    jpatprop_node, jprop_key, jobj_key);
